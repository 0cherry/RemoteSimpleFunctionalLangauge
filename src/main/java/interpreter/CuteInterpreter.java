...
public class CuteInterpreter {
    ...
    private Node lookupVariables(IdNode idNode) {
        ...
        if(defined_expr == null) {
            errorLog("define Error : [" + idNode.getIdString() + "] not defined");
            return null;
        }
        else
            return defined_expr;
    ...
    private Node runList(ListNode list) {
        ...
        // run defined expression
            ...
            errorLog("runList Error : Not a operator");
            return null;
        }
    }
    ...
    private Node runFunction(ListNode list) {
        ...
        switch (function.funcType)
        {
            ...
            case EXIT:
                return exit(rhs);
            case IMPORT:
//                return import_code(rhs);
            default:
                errorLog("runFunction op Error");
                return null;
        }
    }
    ...
    private Node cond(ListNode rhs) {
        ...
        if(!(condition instanceof BooleanNode)) {
            errorLog("[cond] condition can be BooleanNode");
            return null;
        }
    ...
    private Node eq_q(ListNode rhs) {
        ...
        Node eq_q_operand1 = runExpr(rhs.car());
        Node eq_q_operand2 = runExpr(rhs.cdr().car());
    ...
    private Node null_q(ListNode rhs) {
        ...
        if(null_q_operand instanceof ListNode) {
            if(((ListNode) null_q_operand).isEmpty())
                return BooleanNode.TRUE_NODE;
            else
                return BooleanNode.FALSE_NODE;
        }
    ...
    private Node cdr(ListNode rhs) {
        ...
        if(cdr_operand instanceof ListNode) {
            if(((ListNode) cdr_operand).isEmpty()) {
    ...
    private Node car(ListNode rhs) {
        ...
        if(car_operand instanceof ListNode) {
            if(((ListNode) car_operand).isEmpty()) {
    ...
    private Node runBinary(ListNode list) {
        ...
        {
            case MINUS:
                return minus(rhs.car() ,rhs.cdr());
            case PLUS:
                return plus(rhs.car(), rhs.cdr());
            case TIMES:
                return times(rhs.car(), rhs.cdr());
            case DIV:
                return div(rhs.car(), rhs.cdr());
            case LT:
                return lt(rhs.car(), rhs.cdr());
            case GT:
                return gt(rhs.car(), rhs.cdr());
            case EQ:
                return eq(rhs.car(), rhs.cdr());
            default:
