...
public class CuteParserServer implements Runnable {
    private final static int CUTEPARSER_PORT = 4002;

    public void run() {
        System.out.println("CuteParser Server Start");
        ServerSocket server;

        System.out.println("-------waiting for connection------");
        try{
            server = new ServerSocket(CUTEPARSER_PORT);
            while(true) {
    ...
    class ServerHandler extends Thread {
        ...
        private CuteParser cp = new CuteParser();
        ...
        public void run() {
            try {
                while (true) {
                    // 클라이언트로부터 받은 입렫 데이터를 CuteParse
                    Object receivedData = ois.readObject();
                    Node sendData = null;
                    if (receivedData == null)
                        break;
                    if (receivedData instanceof ArrayList) {
                        cp.setTokenList((ArrayList) receivedData);
                        sendData = cp.parseExpr();
                    }

                    // CuteParse 결과 클라이언트에 보내기
                    oos.reset();
